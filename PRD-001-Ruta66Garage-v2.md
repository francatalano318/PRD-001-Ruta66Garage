...
* Transmisión
* Otro
* Sin clasificar (valor asignado únicamente cuando la clasificación automática no pudo completarse; ver RF-23)
...
* Media
* Baja
* Sin asignar (valor asignado únicamente cuando la clasificación automática no pudo completarse; ver RF-24)
...
El sistema debe informar al usuario cuando la clasificación automática no pueda completarse (por falla o indisponibilidad del servicio de IA).

### RF-23

Ante una falla o indisponibilidad del servicio de clasificación automática, el sistema debe crear la orden igualmente, asignando la categoría "Sin clasificar", para que el usuario pueda completarla manualmente sin que la falla del servicio de IA bloquee el registro del trabajo.

### RF-24

Ante una falla o indisponibilidad del servicio de clasificación automática, el sistema debe crear la orden sin prioridad asignada ("Sin asignar"), para que el usuario pueda asignarla manualmente sin que la falla del servicio de IA bloquee el registro del trabajo.
...
El conjunto de prueba debe cumplir con lo siguiente:

* **Quién etiqueta:** las incidencias deben ser etiquetadas manualmente por personal del taller con experiencia en diagnóstico mecánico (o, en su defecto, por el/los usuario(s) referente(s) del producto en conjunto con el taller), de forma que la categoría y prioridad "correctas" reflejen criterio experto y no una interpretación arbitraria.
* **Cómo se valida la representatividad:** el set debe construirse a partir de descripciones reales de incidencias (históricas o relevadas directamente con el taller). Su representatividad se valida comparando, categoría por categoría y prioridad por prioridad, la proporción de casos en el set contra la proporción observada en la operación real del taller durante los últimos 3 meses (o todo el historial disponible, si es menor a 3 meses): la desviación no debe superar los 10 puntos porcentuales en ninguna categoría ni prioridad.
...
**Dado** dos órdenes creadas de manera consecutiva
**Cuando** finaliza la creación de cada una
**Entonces** ambas órdenes poseen un identificador y los dos identificadores son distintos entre sí.
...
**Dado** la descripción "el vehículo hace ruido al frenar" (caso no ambiguo)
**Cuando** se crea la orden
**Entonces** la categoría asignada es "Frenos".

> **Nota:** este AC ilustra el comportamiento esperado en un caso no ambiguo; no garantiza el comportamiento del clasificador en el 100% de los casos. La precisión real del modelo se mide de forma agregada en RNF-04.

---

### AC-07 (RF-07)

**Dado** la descripción "el vehículo no responde al pedal de freno" (caso no ambiguo)
**Cuando** se crea la orden
...
**Dado** una falla o indisponibilidad del servicio de clasificación automática
**Cuando** el usuario intenta crear una orden
**Entonces** el sistema informa al usuario que la clasificación automática no pudo completarse.
...
**Dado** una falla o indisponibilidad del servicio de clasificación automática
**Cuando** se crea la orden
**Entonces** el sistema crea la orden con categoría "Sin clasificar".
...
**Dado** una falla o indisponibilidad del servicio de clasificación automática
**Cuando** se crea la orden
**Entonces** la orden se crea sin prioridad asignada.
...
* Soporte multi-taller o multi-usuario.
...
**Mitigación:** RF-08 y RF-09 permiten corregir manualmente categoría y prioridad; RF-10 y RF-11 marcan visualmente qué campos fueron asignados por IA, para que la revisión humana se enfoque en esos casos.

### R-02

Los usuarios pueden ingresar descripciones insuficientes para una clasificación precisa.

**Mitigación:** RF-21 exige un mínimo de 10 caracteres en la descripción. Si el problema persiste en producción, evaluar agregar guías o ejemplos en el formulario de carga (mejora futura, no incluida en esta versión).

### R-03

La indisponibilidad del servicio de IA puede impedir la clasificación automática.

**Mitigación:** ante una falla, RF-22/AC-22 informan al usuario, RF-23/AC-23 permiten crear la orden con categoría "Sin clasificar" y RF-24/AC-24 permiten crearla sin prioridad asignada, de forma que el usuario complete ambos campos manualmente y el registro del trabajo no quede bloqueado.

### R-04

El conjunto de prueba utilizado para medir precisión puede no representar casos reales del taller.

**Mitigación:** RNF-04 especifica quién etiqueta el conjunto de prueba (personal con criterio experto) y cómo se valida su representatividad (comparando su distribución de categorías/prioridades/ambigüedad contra la operación real del taller).
